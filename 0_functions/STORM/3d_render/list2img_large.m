function [In, imaxes] = list2img_large(mlist,varargin)
%--------------------------------------------------------------------------
...
for c=chns   
    ...
    maxint = 0; 
     Iz = zeros(H,W,Zs,'single');          % adding single
     for k=1:Zs 
         I0 = zeros(H,W,'single');         % adding single          
      ...
      if autocontrast
        %Iz = uint16((2^16)*(double(Iz)./double(maxint))); % normalize
         %Iz = uint16((2^16)*(double(Iz)/double(maxint))); % normalize
         Iz = uint16((2^12)*(double(Iz)/double(maxint)));
      else
