function [Io,Io4d] = Ncolor_zhen_large(I,varargin)
% ------------------------------------------------------------------------
% Ncolor(I);  
...
if numColors == 1 && size(clrmap,1) > 10;
    ...
    Io = zeros(h,w,3,class(I));  %change to single
    Io4d=zeros(h,w,3,class(I));  %change to single
    try
        for c=1:numColors
            for cc = 1:3
                Io(:,:,cc) = Io(:,:,cc) + I(:,:,c)*clrmap(c,cc);  
                Io4d(:,:,cc,c)=I(:,:,c)*clrmap(c,cc);
            end             
        end
    catch er
