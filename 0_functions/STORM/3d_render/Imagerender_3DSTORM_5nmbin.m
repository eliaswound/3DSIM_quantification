...
%--------------------------------------------------------------------------
  ...
  binsize=5; % rendered pixel size in xy, unit nm
...
%--------------------------------------------------------------------------
 ...
 %-------------------------------------------------------------------------
  ...
  [Io,Io4d]=Ncolor_zhen_large(In{1,1}); 
 ...
 %------------------------------------------------------------------------
   ...
   options2.big=true;
   for i=1:Zsteps
       saveastiff(Io4d(:,:,:,i), [file(1:end-4) '_rgb.tif'], options);
       saveastiff(In{1,1}(:,:,i),[file(1:end-4) '.tif'],options2);
   end
   imwrite(Io, [file(1:end-4) 'zstep' num2str(zsteps) 'binsize' num2str(binsize) 'zrange' num2str(zrange(1)) '-' num2str(zrange(2)) '_zproj_rgb.tif'], 'WriteMode', 'overwrite', 'Compression','none');
