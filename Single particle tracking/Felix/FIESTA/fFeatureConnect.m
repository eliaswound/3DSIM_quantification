function [MolTrack,FilTrack,abort]=fFeatureConnect(Objects,Config,StatusNr)
...
if StartFrame==0
    nObj=length(Objects);
    lObjects=double([Objects{nObj}.length]);
    pMol=1;
    pFil=1;
...
for k=StartFrame:EndFrame+1
    if k<=EndFrame
        nObj=length(Objects{k});
        if nObj==0
            Mol(k,:)=0;
            Fil(k,:)=0;
            Data{k}=[];
        else
            lObjects=double([Objects{k}.length]);
            if k==StartFrame
            ...
            Data{k}=zeros(nObj,5);
            Data{k}(:,1)=k;
            Data{k}(:,2)=[Objects{k}.time];
            Data{k}(:,3)=double([Objects{k}.com_x]);
            Data{k}(:,4)=double([Objects{k}.com_y]);        
            int=double([Objects{k}.height]);
            len=double([Objects{k}.length]);
    ...
    iTrack=k+1;
    timeleft=etime(clock,StartTime)/(k-StartFrame+1)*(EndFrame-k+2);
    s=sprintf('Connecting Tracks - Frame:  %d/%d',k+1,EndFrame);
    workbar((k-StartFrame+1)/(EndFrame-StartFrame+2),s,'Progress',timeleft) 
    if StatusNr>0
        StatusC=(k-StartFrame+1)/(EndFrame-StartFrame+2); %#ok<NASGU>
        TimeC=timeleft; %#ok<NASGU>
        try
            save([DirServer 'Status' filesep 'FiestaStatus' num2str(StatusNr) '.mat'],'StatusC','TimeC','-append');
        catch
        end
    end
end
...
for k=StartFrame+1:EndFrame-1
    ...
    timeleft=etime(clock,StartTime)/(k-StartFrame)*(EndFrame-1-k);
    s=sprintf('Postprocessing Tracks - Frame:  %d/%d',k,EndFrame-1);
    workbar((k-StartFrame)/(EndFrame-StartFrame-1),s,'Progress',timeleft) 
    if StatusNr>0
        StatusP=(k-StartFrame)/(EndFrame-StartFrame-1); %#ok<NASGU>
        TimeP=timeleft; %#ok<NASGU>
        try
            save([DirServer 'Status' filesep 'FiestaStatus' num2str(StatusNr) '.mat'],'StatusP','TimeP','-append');
        catch
        end
    end    
end
...
for i=1:size(Obj,2)
    ...
    if ~isempty(quadruples)
        ...
        lastY=[];
        mIntLen=[];
        if best(1)==0&&isinf(best(2)) && isinf(best(3))
        ...
        elseif isinf(best(1))&&isinf(best(2))
            ...
            %if first three or four poins not in track, start new track with 3 points
                        ...
                        for j=1:length(Start_Track{m})
                            ...
                            if norm(x1-x2)+norm(x2-x3)+norm(x3-x4)<max([rmax Config.PixSize])
                                ...
                                if n==Config.Connect.NumberVerification
                                    if m-(i-k+1)<Config.Connect.MaxBreak
