...
function MainGuiCreate(DirServer)
close all hidden
global Config;
global FiestaDir;
%title for the figure

hMainGui.Version='Version 0.04.0001';
hMainGui.Date='December 03rd, 2009';
...
hMainGui.fig= figure('Units','normalized','DockControls','off','IntegerHandle','off','Name',hMainGui.Name,'MenuBar','none',...
                     ...
                     'Visible','on','NextPlot','add','Renderer','painters');%'Interruptible','off','BusyAction','cancel');
...
hMainGui.hPan = pan(hMainGui.fig);
hMainGui.hZoom = zoom(hMainGui.fig);
...
hMainGui=DefStruct(hMainGui,DirServer);
Config=fLoad(FiestaDir.Config,'Config');
...
FiestaDir.Load=[];
FiestaDir.Save=[];
FiestaDir.Stack=DirRoot;
FiestaDir.Config=[DirRoot 'FIESTA' filesep 'Config.mat'];
...
function hMainGui=DefStruct(hMainGui,DirServer)
global Molecule;
global Filament;
global Stack;
global KymoTrackMol;
...
global ServerQueue;
global FiestaDir;
hMainGui.Region = struct('Area',{},'Check',{},'X',{},'Y',{});
...
KymoTrackMol=struct('Index',{},'Track',{},'plot',{},'plotSelectB',{},'plotSelectW',{});
KymoTrackFil=struct('Index',{},'Track',{},'plot',{},'plotSelectB',{},'plotSelectW',{});
OffsetMap = struct('RedXY',[],'GreenXY',[],'Match',[],'VisibleWarning',0);
setappdata(hMainGui.fig,'OffsetMap',OffsetMap);
Queue=[];
FiestaDir.Server=DirServer;
if ~isempty(DirServer)
    ServerQueue = fLoad([DirServer 'Queue' filesep 'FiestaQueue.mat'],'ServerQueue');
...
%get size in pixel for aspect ratio
axes(hMainGui.LeftPanel.pNorm.aScaleBar);
set(hMainGui.LeftPanel.pNorm.aScaleBar,'Units','pixels');
set(hMainGui.LeftPanel.pNorm.aScaleBar,'Units','normalized');
...
axes(hMainGui.LeftPanel.pThresh.aScaleBar);
image(Scale)
axis off
...
axes(hMainGui.LeftPanel.pRedNorm.aScaleBar);
image(ScaleRGB);
axis off

%draw RGB Thresh Scalebar red
axes(hMainGui.LeftPanel.pRedThresh.aScaleBar);
image(ScaleRGB);
axis off
...
axes(hMainGui.LeftPanel.pGreenNorm.aScaleBar);
image(ScaleRGB);
axis off

%draw RGB Thresh Scalebar green
axes(hMainGui.LeftPanel.pGreenThresh.aScaleBar);
image(ScaleRGB);
axis off
...
if ~isempty(strInfo)
    ...
    if all(Selected(List)<2)||strcmp(hMainGui.CurrentAxes,'Kymo')
        if isempty(hMainGui.Plots.TrackInfo)
            hMainGui.Plots.TrackInfo=text(cp(1)-dx,cp(2),strInfo(1:p),'Parent',hMainGui.CurrentAxesHandle,'BackgroundColor',[.7 .9 .7],'Tag','TrackInfo','UIContextMenu',hMainGui.Menu.ctTrack(ct).menu,'UserData',TrackInfo);
        else
            set(hMainGui.Plots.TrackInfo,'String',strInfo(1:p),'Position',[cp(1)-dx,cp(2)],'UIContextMenu',hMainGui.Menu.ctTrack(ct).menu,'UserData',TrackInfo);
        end
    else
        k=find(Selected==2,1);
        set(Object(k).pTrack,'UIContextMenu',hMainGui.Menu.ctTrack(2).menu,'UserData',TrackInfo);
    end
else
...
global KymoTrackFil;
fShared('ReturnFocus');
if isempty(findobj('Tag','hMergeGui'))&&isempty(findobj('Tag','hStatsGui'))&&isempty(findobj('Tag','hConfigGui'))&&isempty(findobj('Tag','hExportGui'))
    hMainGui=getappdata(0,'hMainGui');
    set(0,'CurrentFigure',hMainGui.fig);
    cpKymo=get(hMainGui.RightPanel.pTools.aKymoGraph,'currentpoint');
    cpKymo=cpKymo(1,[1 2]);
        ...
        if strcmp(hMainGui.CurrentAxes,'View')
            ...
            if all(hMainGui.Values.CursorDownPos==0)&&~isempty(Stack)
                ...
                elseif ~isempty(strfind(hMainGui.CursorMode,'Scan'))
                    plotScan=findobj('Tag','plotScan','-or','Tag','plotLineScan');
                    if ~isempty(plotScan)
                        delete(plotScan);
                    end
                    if ~isempty(KymoTrackMol)
...
global Filament;
hMainGui=getappdata(0,'hMainGui');
if ~strcmp(hMainGui.CurrentAxes,'none')&&all(hMainGui.Values.CursorDownPos==0)
...
if isempty(findobj('Tag','hMergeGui'))&&isempty(findobj('Tag','hStatsGui'))&&isempty(findobj('Tag','hConfigGui'))&&isempty(findobj('Tag','hExportGui'))
    ...
    elseif strcmp(get(hMainGui.fig,'SelectionType'),'extend')
        ...
        if ~isempty(hMainGui.CurrentKey) 
             tag=get(get(gco,'Parent'),'Tag');            
             if strcmp(tag,'MoleculePan')&&strcmp(hMainGui.SelectMode,'Molecule')
                 SelectObject(hMainGui,'Molecule',get(gco,'UserData')*1i,'extend');
             elseif strcmp(tag,'FilamentPan')&&strcmp(hMainGui.SelectMode,'Filament')
                 SelectObject(hMainGui,'Filament',get(gco,'UserData')*1i,'extend');
             elseif strcmp(tag,'LocalPan')&&strcmp(hMainGui.SelectMode,'LocalQueue')
...
hMainGui=getappdata(0,'hMainGui');
if strcmp(evnt.Key,'shift')&&strcmp(hMainGui.CursorMode,'Pan')
    hMainGui.Values.CursorDownPos(:)=0;    
    hMainGui.CursorMode='Normal';
    set(hMainGui.fig,'pointer','arrow');
end
