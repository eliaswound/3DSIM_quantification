...
set(hMainGui.hZoom,'Enable','off');
set(hMainGui.hPan,'Enable','off');
SetZoom(hMainGui);

function ToolPan(hMainGui)
global PanInfo;
if isempty(PanInfo)
    warndlg({'You can only use the pan function','while pressing down the scroll wheel','or the left and right button simutaneously'},'Attention','modal');
    set(hMainGui.ToolBar.ToolPan,'State','off');
    fShared('ReturnFocus');
else
    set(hMainGui.ToolBar.ToolCursor,'State','off');
    set(hMainGui.ToolBar.ToolPan,'State','on');
    set(hMainGui.ToolBar.ToolZoomIn,'State','off');
    set(hMainGui.ToolBar.ToolRegion,'State','off');
    set(hMainGui.ToolBar.ToolRectRegion,'State','off');
    hMainGui.Values.CursorDownPos(:)=0;
    hMainGui.CursorMode='Zoom';
    setappdata(0,'hMainGui',hMainGui);
    fRightPanel('AllToolsOff',hMainGui);
    hMainGui=getappdata(0,'hMainGui');    
    set(hMainGui.hZoom,'Enable','off');
    set(hMainGui.hPan,'Enable','on');
    SetZoom(hMainGui);
end
...
global ZoomInfo;
if isempty(ZoomInfo)
    warndlg({'You can only use the zoom function','by scrolling the mouse wheel up and down '},'Attention','modal');
    set(hMainGui.ToolBar.ToolZoomIn,'State','off');
    fShared('ReturnFocus');
else
    set(hMainGui.ToolBar.ToolCursor,'State','off');
    set(hMainGui.ToolBar.ToolPan,'State','off');
    set(hMainGui.ToolBar.ToolZoomIn,'State','on');
    set(hMainGui.ToolBar.ToolRegion,'State','off');
    set(hMainGui.ToolBar.ToolRectRegion,'State','off');
    hMainGui.Values.CursorDownPos(:)=0;
    hMainGui.CursorMode='Zoom';
