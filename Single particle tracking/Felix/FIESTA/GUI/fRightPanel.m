...
switch func
    ...
    case 'UpdateQueue'
        UpdateQueue(varargin{1},varargin{2},varargin{3},varargin{4});
    case 'QueueSlider'
        QueueSlider(varargin{1});        
    case 'ShowAllFil'
...
if FilterIndex>0||strcmp(mode,'Print')==1
    ...
    nZI = interp2(double(Stack{f}),nXI,nYI);
    d(1)=0;
    p=2;    
    I(1)=mean(nZI);
    X=nXI;
    ...
    Z = interp2(double(Stack{f}),X,Y);
    for i = 1:p-1
        I(i) = mean(Z((i-1)*2*hMainGui.Values.ScanSize+1:(i-1)*2*hMainGui.Values.ScanSize+2*hMainGui.Values.ScanSize));
    end
    if FilterIndex==1||strcmp(mode,'Print')==1
        h=figure('PaperUnits','centimeter','PaperType','A4','PaperPositionMode','manual','PaperPosition',[0 0 29.7 21]);
        plot(d,I,'b-');
        xlabel('Intensity vs. Distance in µm');
        if FilterIndex==1
            print(h,'-djpeg','-noui',file);  
        else
    ...
    elseif FilterIndex==2
        ...
        fprintf(fh,'Distance in µm\tIntensity');
        for j=1:length(d)
            fprintf(fh,'%f\t%f\n',d,I);
        end
        fclose(fh);
...
if ~isempty(Drift)
    nMol=length(Molecule);
    for i=1:nMol
        if get(hMainGui.RightPanel.pData.cMolDrift,'Value')==1||Molecule(i).Drift==1
            Molecule(i)=CalcDrift(Molecule(i),Drift,0);
            Molecule(i).Drift=0;
        end   
    end
    set(hMainGui.RightPanel.pData.cMolDrift,'Value',0);   
    nFil=length(Filament);
    for i=1:nFil
        if get(hMainGui.RightPanel.pData.cFilDrift,'Value')==1||Filament(i).Drift==1
            Filament(i)=CalcDrift(Filament(i),Drift,0);
            Filament(i).Drift=0;
        end   
    end
    set(hMainGui.RightPanel.pData.cFilDrift,'Value',0); 
...
for n=1:length(Object)
    ...
    if ~isempty(k)
        ...
        set(KymoObject(k).plotSelectB,'Visible','off');        
        set(KymoObject(k).plotSelectW,'Visible','off');   
    end  
end
if strcmp(Mode,'Molecule')
    Molecule=Object;
...
if ~isempty(hMainGui.KymoImage)
    if ~isempty(Molecule)
        delete([KymoTrackMol.plot]);
        delete([KymoTrackMol.plotSelectB]);
        delete([KymoTrackMol.plotSelectW]);        
        [hMainGui,KymoTrackMol]=ShowTracksKymo(hMainGui,Molecule,hMainGui.Scan.InterpX,hMainGui.Scan.InterpY,s,e,hMainGui.Scan.lx,hMainGui.Scan.ux,hMainGui.Scan.ly,hMainGui.Scan.uy,hMainGui.Kymo.xOffset,hMainGui.Kymo.yOffset);
    end
    if ~isempty(Filament)
        delete([KymoTrackFil.plot]);
        delete([KymoTrackFil.plotSelectB]);
        delete([KymoTrackFil.plotSelectW]);                
        [hMainGui,KymoTrackFil]=ShowTracksKymo(hMainGui,Filament,hMainGui.Scan.InterpX,hMainGui.Scan.InterpY,s,e,hMainGui.Scan.lx,hMainGui.Scan.ux,hMainGui.Scan.ly,hMainGui.Scan.uy,hMainGui.Kymo.xOffset,hMainGui.Kymo.yOffset);
    end
end
setappdata(0,'hMainGui',hMainGui);
...
KymoTrackObj=struct('Index',{},'Track',{},'plot',{},'plotSelectB',{},'plotSelectW',{});
nTrack=1;
newX=mean(X,1);
newY=mean(Y,1);
for idx=1:length(Objects)
    ...
    KymoTrack=[];
    for n=1:length(k)
        [m,t]=min(sqrt( (newX-OX(IN(k(n)))).^2 + (newY-OY(IN(k(n)))).^2));
        KymoTrack(n,:)=[Objects(idx).Results(IN(k(n)),1)-s t];
    end
...
function QueueSlider(hMainGui)
global Queue;
global ServerQueue;
Mode=get(gcbo,'UserData');
if strcmp(Mode,'Local');
    UpdateQueue(hMainGui.RightPanel.pQueue.LocList,Queue,hMainGui.RightPanel.pQueue.sLocList,Mode);
else
    UpdateQueue(hMainGui.RightPanel.pQueue.SrvList,ServerQueue,hMainGui.RightPanel.pQueue.sSrvList,Mode);    
end    
fShared('ReturnFocus');

function UpdateQueue(hQueue,Queue,slider,mode)
global FiestaDir;
hMainGui=getappdata(0,'hMainGui');
l=length(Queue);
if l>9
    ...
    if strcmp(get(slider,'Enable'),'on')==1
        ...
        set(slider,'sliderstep',slider_step,...
         'max',l-8,'min',1,'Value',v)
    else
        set(slider,'sliderstep',slider_step,...
         'max',l-8,'min',1,'Value',l-9,'Enable','on')
    end
...
for i=1:QueueLength
    bgcolor=get(slider,'Background');    
    fgcolor=[1 1 1];
    switch(Queue(i+QueueBegin).Selected)
        case 1
            bgcolor=[0 0 0.8];
        case 0
            fgcolor=[0 0 0];
    ...
    if strcmp(mode,'Local')
        ...
        set(hQueue.Name(i),'Enable','inactive','BackgroundColor',bgcolor,'ForegroundColor',fgcolor,'UIContextMenu',hMainGui.Menu.ctListLoc,'String',Queue(i+QueueBegin).StackName);
        set(hQueue.File(i),'Enable','inactive','BackgroundColor',bgcolor,'ForegroundColor',fgcolor,'UIContextMenu',hMainGui.Menu.ctListLoc,'String',Queue(i+QueueBegin).Directory);
        set(hQueue.Back(i),'BackgroundColor',bgcolor,'UIContextMenu',hMainGui.Menu.ctListLoc); 
    else
        set(hQueue.Pan(i),'Visible','on');    
        set(hQueue.Name(i),'Enable','inactive','BackgroundColor',bgcolor,'ForegroundColor',fgcolor,'String',Queue(i+QueueBegin).StackName);
...
if strcmp(mode,'Local')
    ...
    for n=1:2
        try
            [TimeT,StatusT,TimeC,StatusC,TimeP,StatusP,QueueNr] = fLoad([FiestaDir.Server 'Status' filesep 'FiestaStatus' num2str(n) '.mat'],'TimeT','StatusT','TimeC','StatusC','TimeP','StatusP','QueueNr');
        catch
            QueueNr=0;
        end
        if QueueNr~=0
           set(hQueue.File(QueueNr),'String',{sprintf('Tracking: %3.0f%% (%s)',StatusT*100,sec2timestr(TimeT)),...
                                              sprintf('Connecting: %3.0f%% (%s)',StatusC*100,sec2timestr(TimeC)),...
                                              sprintf('Postprocessing: %3.0f%% (%s)',StatusP*100,sec2timestr(TimeP))});
        end
    end
end
...
if FileName~=0
    ...
    UpdateQueue(hMainGui.RightPanel.pQueue.LocList,Queue,hMainGui.RightPanel.pQueue.sLocList,'Local');    
end
fShared('ReturnFocus');
