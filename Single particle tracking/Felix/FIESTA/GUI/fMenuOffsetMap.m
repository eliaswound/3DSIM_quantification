...
if OffsetMap.VisibleWarning==0
    warndlg('Selected tracks will be hidden','FIESTA Warning');
...
if size(OffsetMap.RedXY,1)~=size(OffsetMap.GreenXY,1)
    errordlg('Number of offset points in the channels do not match','FIESTA Error');
    return;
else
    OffsetMap.Match=[];
...
if ~isempty(OffsetMap.Match)
    ...
    k = find([Molecule.Selected]==1);
    for n = k
        Molecule(n).Results(:,3)=Molecule(n).Results(:,3) * X(1) + Molecule(n).Results(:,4) * X(2) + X(5);
        Molecule(n).Results(:,4)=Molecule(n).Results(:,3) * X(3) + Molecule(n).Results(:,4) * X(4) + X(6);
    end
