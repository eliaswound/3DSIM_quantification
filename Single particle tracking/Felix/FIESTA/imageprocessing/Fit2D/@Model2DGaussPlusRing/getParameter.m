...
%   else
  ...
  if length( model.guess.r )<2
    model.guess.r(2) = sqrt(model.guess.w(1))*4;
  end
  ...
  if length( model.guess.h )<2
    model.guess.h(2) = mean( interp2( fit_pic, c(1)+model.guess.r(2)*cos(phi), c(2)+model.guess.r(2)*sin(phi), '*nearest' ) - double( data.background ));
  end  
