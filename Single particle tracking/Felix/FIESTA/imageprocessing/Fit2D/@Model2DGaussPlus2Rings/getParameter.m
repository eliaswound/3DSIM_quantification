...
%   else
  ...
  if isempty( model.guess.h )
    model.guess.h = interp2( fit_pic, c(1), c(2), '*nearest' ) - min(min(fit_pic))+1;
  end
  ...
  if length( model.guess.h )<3
    model.guess.h(3) = mean( interp2( fit_pic, c(1)+model.guess.r(3)*cos(phi), c(2)+model.guess.r(3)*sin(phi), '*nearest' ) - double( data.background ));
  end  
%   end
