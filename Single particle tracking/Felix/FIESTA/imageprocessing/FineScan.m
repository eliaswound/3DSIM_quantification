...
%     end
  ...
  FIT_AREA_FACTOR = 2.5; %<< factor determining the size of the area used for fitting
  ...
  while i <= numel( objects )
    heights = double( [ objects(i).p.h ] );
    objects(i).p( heights < height_thresh ) = [];
    if isempty( objects(i).p )
      objects(i) = [];
...
%         cluster_rects(j,1:2)
    ...
    for j = cluster_ids
      region(1:2) = min( region(1:2), min( dist( j, 2:3 ), dist( j, 4:5 ) ) );
      region(3:4) = max( region(3:4), max( dist( j, 2:3 ), dist( j, 4:5 ) ) );
    end
    ...
    for obj = obj_ids
      ...
      if numel( objects(obj).p ) == 1 % check, if it is a point-like object
        ...
        guess(end).idx = 1;
        guess(end).x = objects(obj).p(1).x;
      elseif numel( objects(obj).p ) == 2 % check, if it is a short filament
        guess(end).model = 't';
        guess(end).idx = 1;
        guess(end).x = [ objects(obj).p(1).x ; objects(obj).p(2).x ];
      else % its an elongated object
        start_inside = inRectangle( double( objects(obj).p(1).x ), region );
        end_inside = inRectangle( double( objects(obj).p(end).x ), region );
        if start_inside
          if end_inside % assume whole filament is inside!
            guess(end).model = 't';
            guess(end).idx = 1;
            guess(end).x = [ objects(obj).p(1).x ; objects(obj).p(end).x ];
          else % only start point is inside
            guess(end).model = 'e';
            guess(end).idx = 1;
            guess(end).x = objects(obj).p(1).x;
            guess(end).o = objects(obj).p(1).o + pi;
          end
        else
          if end_inside % only end point is inside
            guess(end).model = 'e';
            guess(end).idx = numel( objects(obj).p );
            guess(end).x = objects(obj).p(end).x;
            guess(end).o = objects(obj).p(end).o;
          else % only middle points are inside
            % find point, which is closet to center
            dist2center = Inf;
            p_idx = NaN;
            for j =  1 : numel( objects(obj).p )
              if inRectangle( double( objects(obj).p(j).x ), region ) && ...
                 norm( objects(obj).p(j).x - region_center ) < dist2center
                p_idx = j;
                dist2center = norm( objects(obj).p(j).x - region_center );
              end
            end
            if isnan( p_idx )
              guess(end) = [];
              ...
              guess(end).model = 'm';
              guess(end).idx = p_idx;
              guess(end).x = objects(obj).p(p_idx).x;
              guess(end).o = objects(obj).p(p_idx).o;
            end
          end
        end
...
%   objects   the extended objects array
  ...
  while k <= numel(objects) % run through all objects
    ...
    if numel( objects(k).p ) == 1 % single point
      if isempty( objects(k).p(1).b ) % has not been fitted
    ...
    elseif numel( objects(k).p ) == 2 % small filament
      if isempty( objects(k).p(1).b ) || isempty( objects(k).p(2).b ) % has not been fitted
      ...
      i = 1;
      fit_mp = 0; %<< counts the number of unfitted intermediate points
      while i <= numel( objects(k).p ) % run through all points
        if ~isempty( objects(k).p(i).b ) % has already been fitted
          i = i + 1;
          continue;
        end
        % otherwise: fit!
        ...
        elseif i == numel( objects(k).p ) % end point
          ...
          fit_mp = fit_mp + 1; % increase count of intermediate points
          if fit_mp > 2 * params.fit_size % assuming the points are approx. 1 pixel apart
            p.o = p.o + pi;
            [ data, CoD ] = Fit2D( 'm', p, params );
            if CoD > params.min_cod % fit went well
              objects(k).p(i) = data;
              fit_mp = 0; % reset intermediate points count
            else % bad fit result
              objects(k).p(i) = [];
              error_events.middlepoint_cod_low = error_events.middlepoint_cod_low + 1;
              continue;
            end
          else % delete this point, because we dont need that many intermediate points
            objects(k).p(i) = [];
            continue;
          end
          
        end
        i = i + 1; % step to next point
...
function inside = inRectangle( point, rect )
...
%           rectangle
  ...
  if isempty( rect )
    ...
    point = repmat( point, size(rect,1), 1 );
    inside = all( point >= rect(:,1:2) ) & all( point <= rect(:,3:4) );
  end
end
